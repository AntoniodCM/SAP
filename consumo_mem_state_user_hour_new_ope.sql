SELECT
         ...
         sum (DISTINCT MEM_PER_EXEC_GB) "SUMA_TOTAL_GB",
         count (DISTINCT STATEMENT_ID) "NUM_STATEMENTS",
         count (OPERATION) "NUM_OPERACIONES",
         max (HORA) "HORA",
...
SELECT HOST, PORT, DB_USER, MEMORY_SIZE/(1024*1024*1024) AS MEM_PER_EXEC_GB, statement_id, OPERATION, right (concat ('0' , RTRIM(EXTRACT (HOUR FROM (add_seconds (TO_TIMESTAMP (CURRENT_TIMESTAMP), 3600*23) ))) ),2) || ':00:00' AS HORA
...
GROUP BY HOST, PORT, DB_USER, MEMORY_SIZE, STATEMENT_ID, OPERATION
ORDER BY DB_USER
)
group by db_user
ORDER BY DB_USER;
